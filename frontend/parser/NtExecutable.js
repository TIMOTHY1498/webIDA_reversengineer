...
export default class NtExecutable {
    ...
    _nh;
    _dda;
    constructor(_headers, _sections, _ex) {
     ...
     */
    static createEmpty(is32Bit = false, isDLL = true) {
    ...
    static from(bin, options) {
        ...
        const sections = [];
        const tempSectionHeaderBinary = allocatePartialBinary(bin, secOff, secCount * ImageSectionHeaderArray.itemSize);
        const secArray = ImageSectionHeaderArray.from(tempSectionHeaderBinary, secCount, 0);
        let lastOffset = roundUp(secOff + secCount * ImageSectionHeaderArray.itemSize, fileAlignment);
        ...
        secArray.forEach((info) => {
            if (!info.pointerToRawData || !info.sizeOfRawData) {
                ...
                const secBin = allocatePartialBinary(bin, info.pointerToRawData, info.sizeOfRawData);
                sections.push({
                    info,
                    data: secBin,
                });
                const secEndOffset = roundUp(info.pointerToRawData + info.sizeOfRawData, fileAlignment);
     ...
     */
    setSectionByEntry(entry, section) {
        const sec = section
    ...
    generate(paddingSize) {
        ...
        const secArray = ImageSectionHeaderArray.from(bin, this._sections.length, secOff);
    ...
    rearrangeSections(rawAddressStart, rawDiff, virtualAddressStart, virtualDiff) {
    ...
    replaceSectionImpl(virtualAddress, info, data) {
        const len = this._sections.length;
        for (let i = 0; i < len; ++i) {
...
//# sourceMappingURL=NtExecutable.js.map