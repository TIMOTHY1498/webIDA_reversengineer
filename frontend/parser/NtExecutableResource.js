...
import { binaryToString, cloneObject, copyBuffer, roundUp, stringToBinary, } from './functions.js';
...
export default class NtExecutableResource {
     ...
     */
    sectionDataHeader = null;
    originalSize = 0;
    constructor() { }
    parse(section, ignoreUnparsableData) {
        if (!section.data) {
            return;
        }
        ...
        const res = [];
        const cb = (t, n, l) => {
            const off = view.getUint32(l.dataOffset, true) -
     ...
     */
    static from(exe, ignoreUnparsableData = false) {
        const secs = []
     ...
     */
    getResourceEntriesAsString(type, id) {
        return this.entries
     ...
     */
    replaceResourceEntryFromString(type, id, lang, value) {
        const entry = {
            type,
            id,
            lang,
     ...
     */
    removeResourceEntry(type, id, lang) {
     ...
     */
    generateResourceData(virtualAddress, alignment, noGrow = false, allowShrink = false) {
     ...
     * @param allowShrink Set true to allow shrinking resource section (if the data size is less than original)
     */
    outputResource(exeDest, noGrow = false, allowShrink = false) {
        // make section data
        ...
        const data = this.generateResourceData(0, fileAlign, noGrow, allowShrink);
...
//# sourceMappingURL=NtExecutableResource.js.map