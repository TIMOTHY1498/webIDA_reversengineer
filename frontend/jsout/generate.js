...
const DOS_STUB_SIZE = roundUp(ImageDosHeader.size + DOS_STUB_PROGRAM.length, 0x80);
...
export function fillPeHeaderEmptyData(bin, offset, totalBinSize, is32Bit, isDLL) {
    ...
    const oh = (is32Bit ? ImageOptionalHeader : ImageOptionalHeader64).from(_bin, _offset + 4 + ImageFileHeader.size);
    oh.magic = is32Bit
...
export function makeEmptyNtExecutableBinary(is32Bit, isDLL) {
    const bufferSize = roundUp(DOS_STUB_SIZE + estimateNewHeaderSize(is32Bit), DEFAULT_FILE_ALIGNMENT);
